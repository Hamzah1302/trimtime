...
// --- PERUBAHAN: Import hook React ---
import { useState, useCallback, useMemo } from "react";
import { useParams } from "next/navigation"; // <-- Hook untuk Client Component
...
// --- PERUBAHAN: Import BookingStatus dan data mock ---
import {
	bookingDetails,
	bookingList, // <-- Import bookingList untuk disinkronkan
	type BookingStatus,
} from "@/data/barber-booking";

// --- (Semua konstanta format, style, dan config tidak berubah) ---
...
> = {
	...
	ongoing: {
		...
		primaryLabel: "Selesaikan Layanan",
		primaryVariant: "default",
...
// --- (Akhir dari konstanta) ---

// --- PERUBAHAN: Menghapus 'async' dan 'props' ---
export default function BarberBookingDetailPage() {
	// --- PERUBAHAN: Menggunakan hook Client Component ---
	const params = useParams();
	const id = typeof params.id === "string" ? params.id : "";
	const bookingKey = id?.toLowerCase?.() ?? "";

	// --- PERUBAHAN: Menggunakan 'useState' untuk data booking ---
	const [booking, setBooking] = useState(
		// Ambil data dari mock 'bookingDetails' saat pertama kali load
		() => bookingDetails[bookingKey]
	);

	// --- PERUBAHAN: Fungsi untuk update status ---
	const handleUpdateStatus = useCallback(
		(newStatus: BookingStatus) => {
			if (!booking) return;

			// 1. Tentukan data baru (progress, label) berdasarkan status baru
			let newProgress = booking.progress;
			let newStatusLabel = "Unknown";
			let newTimelineEntry:
				...
				| undefined = undefined;
			const newTime = new Intl.DateTimeFormat("id-ID", {
				hour: "2-digit",
				minute: "2-digit",
			}).format(new Date());

			// Tentukan data baru berdasarkan status yang dituju
			switch (newStatus) {
				case "confirmed":
					newProgress = 25;
					newStatusLabel = "Terkonfirmasi";
					newTimelineEntry = {
						icon: BadgeCheck,
						title: "Booking Dikonfirmasi",
						description: "Booking telah dikonfirmasi oleh barber.",
						time: newTime,
						status: "completed",
					};
					break;
				case "ongoing":
					newProgress = 50;
					newStatusLabel = "Berjalan";
					newTimelineEntry = {
						icon: CalendarClock,
						title: "Layanan Dimulai",
						description:
							"Barber telah memulai layanan untuk pelanggan.",
						time: newTime,
						status: "completed",
					};
					break;
				case "done":
					newProgress = 100;
					newStatusLabel = "Selesai";
					newTimelineEntry = {
						icon: ClipboardCheck,
						title: "Layanan Selesai",
						description:
							"Layanan telah selesai. Menunggu pembayaran.",
						time: newTime,
						status: "completed",
					};
					break;
				case "cancelled":
					newProgress = 0;
					newStatusLabel = "Dibatalkan";
					newTimelineEntry = {
						icon: XCircle,
						title: "Booking Dibatalkan",
						description: "Booking dibatalkan oleh barber.",
						time: newTime,
						status: "completed",
					};
					break;
			...
			// Buat data booking yang sudah di-update
			const updatedBooking = {
				...booking,
				status: newStatus,
				statusLabel: newStatusLabel,
				progress: newProgress,
				// Tambahkan timeline baru jika ada
			...
			// 2. Update state lokal (agar halaman ini re-render)
			setBooking(updatedBooking);

			// 3. SINKRONISASI: Mutasi data mock global
			// Ini adalah "hack" agar halaman lain bisa melihat perubahan
			if (bookingDetails[bookingKey]) {
				bookingDetails[bookingKey] = updatedBooking;
			}
			const listIndex = bookingList.findIndex(
				(b) => b.id.toLowerCase() === bookingKey
			);
			if (listIndex > -1) {
				// Update data di 'bookingList' juga
				bookingList[listIndex] = {
					...bookingList[listIndex],
					status: newStatus,
				};
			}
	...
	// --- PERUBAHAN: Fungsi untuk Aksi Utama (Tombol Besar) ---
	const handlePrimaryAction = useCallback(() => {
		if (!booking) return;
		const status = booking.status;

		if (status === "pending") handleUpdateStatus("confirmed");
		else if (status === "confirmed") handleUpdateStatus("ongoing");
		else if (status === "ongoing") handleUpdateStatus("done");
		else if (status === "done")
			alert("Aksi 'Kirim Rating' belum berfungsi.");
		else if (status === "cancelled")
			alert("Aksi 'Jadwalkan Ulang' belum berfungsi.");
	}, [booking, handleUpdateStatus]);

	// --- PERUBAHAN: Fungsi Aksi Sekunder (Placeholder) ---
	const handleSecondaryAction = useCallback(() => {
		if (!booking) return;
		alert(
			`Aksi sekunder untuk status: ${booking.status}... (placeholder)`
		);
	}, [booking]);

	// --- PERUBAHAN: Pindahkan kalkulasi 'telHref' ke 'useMemo' ---
	const { telHref, bookingBasePath } = useMemo(() => {
		...
		return {
			...
			bookingBasePath: `/barber/booking/${id}`,
		};
	}, [booking, id]);
						...
						<div className="grid cols-2 gap-8">
							...
							<Card className="border-border/50 shadow-sm">
								...
								<CardContent className="flex flex-col gap-6 text-sm text-muted-foreground">
									{/* --- PERUBAHAN: Tombol di blok ini sekarang terhubung --- */}
									{(() => {
										const statusContent =
