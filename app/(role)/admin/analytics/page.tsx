...
const insightTiles: Array<{ icon: LucideIcon; label: string; value: string; helper: string }> = [
    {
        icon: BarChart3,
                ...
                <div className='relative space-y-6'>
                    <div className='flex flex-col gap-4 rounded-2xl border border-border/50 bg-card/85 p-6 shadow-sm backdrop-blur-sm lg:flex-row lg:items-center lg:justify-between'>
                        <div className='space-y-2'>
                            ...
                            <h1 className='text-3xl font-bold tracking-tight lg:text-4xl'>Layanan populer, lokasi ramai, jam padat</h1>
                            <p className='text-sm text-muted-foreground lg:text-base'>
                                Data digabung dari booking owner, freelancer, dan pelanggan langsung.
                            </p>
                        </div>
                        <Button>
            ...
            <main className='space-y-6 px-5 py-6 lg:px-8 lg:py-8'>
                    ...
                    {insightTiles.map((tile) => (
                        <Card key={tile.label} className='border-border/50 shadow-sm'>
                            <CardHeader>
                                ...
                                <CardTitle className='text-2xl font-bold text-foreground'>{tile.value}</CardTitle>
                ...
                <div className='grid gap-5 lg:grid-cols-[1.4fr_1fr]'>
                    <Card className='border-border/50 shadow-sm'>
                        <CardHeader>
                            <CardTitle className='text-xl font-semibold'>Layanan paling banyak dipesan</CardTitle>
                            <CardDescription>Porsi dari total booking nasional.</CardDescription>
                        </CardHeader>
                        <CardContent className='space-y-3'>
                            {popularServices.map((service) => (
                                <div key={service.name} className='space-y-2 rounded-xl border border-border/40 bg-muted/15 p-4'>
                    ...
                    <Card className='border-border/50 shadow-sm'>
                        <CardHeader>
                            <CardTitle className='text-xl font-semibold'>Slot paling padat</CardTitle>
                            <CardDescription>Gunakan insight ini untuk push notifikasi.</CardDescription>
                        </CardHeader>
                        <CardContent className='space-y-3'>
                            {busySlots.map((slot) => (
                                <div key={slot.slot} className='rounded-xl border border-border/40 bg-muted/15 p-4'>
                                    <p className='text-sm font-semibold text-foreground'>{slot.slot}</p>
                                    <p className='text-xs text-muted-foreground'>Okupansi {slot.occupancy}%</p>
                                </div>
                            ))}
                            <div className='rounded-xl border border-dashed border-primary/40 bg-primary/5 p-4 text-sm text-muted-foreground'>
                                ...
                                <p>Owner bisa otomatis menambah slot & promo saat jam padat.</p>
                            </div>
                        </CardContent>
                    </Card>
                </div>
